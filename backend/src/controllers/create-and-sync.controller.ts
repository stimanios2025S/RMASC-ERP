...
export async function createAndSync(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const pdfData: PdfOrderData = {
      ...
      typeCabine: (order as any).typeCabine ?? null,
      typePorte: (order as any).typePorte ?? null,
      finitionPorteCabine: (order as any).finitionPorteCabine ?? null,
      typeChassisArcade: (order as any).typeChassisArcade ?? null,
      finitionInterieurCabine: (order as any).finitionInterieurCabine ?? null,
      revetementSol: (order as any).revetementSol ?? null,
      largeurPassageLibreMm: (order as any).largeurPassageLibreMm ?? null,
      hauteurUtileCabineMm: (order as any).hauteurUtileCabineMm ?? null,
      typeSuspensionGuidage: (order as any).typeSuspensionGuidage ?? null,
      systemeSurcharge: (order as any).systemeSurcharge ?? null,
