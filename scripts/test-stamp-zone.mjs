...
const STAMP_ZONE = { leftPct: 0.64, bottomPct: 0.015, widthPct: 0.34, heightPct: 0.13 }
...
for (const page of pages) {
  ...
  // Cachet TOUJOURS HORIZONTAL (rotation 90° si image source verticale)
  ...
  console.log(`Page ${width.toFixed(0)}x${height.toFixed(0)}pt — cachet source ${rawW}x${rawH} (${isPortrait ? 'portrait → ROTATION 90°' : 'déjà paysage'})`)
  console.log(`  Zone signature : x=${zoneX.toFixed(1)} y=${zoneY.toFixed(1)} w=${zoneW.toFixed(1)} h=${zoneH.toFixed(1)}`)
  console.log(`  Cachet VISUEL  : x=${(isPortrait ? x - visW : x).toFixed(1)} y=${y.toFixed(1)} w=${visW.toFixed(1)} h=${visH.toFixed(1)} (HORIZONTAL)`)
  console.log(`  Étiquette      : y=${(y + visH + 8).toFixed(1)}, alignée droite du cachet`)
}
