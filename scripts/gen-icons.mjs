#!/usr/bin/env node
// ═══════════════════════════════════════════════════════════════════════════
//  RMASC FACTORY — Génération des icônes PNG professionnelles
//  Zéro dépendance (zlib intégré à Node). Reproduit fidèlement public/images/
//  icon-192.svg (skyline d'ascenseurs) en PNG — requis par iOS pour l'écran
//  d'accueil (apple-touch-icon doit être un PNG 180×180).
//
//  Usage :  node scripts/gen-icons.mjs
//  Sortie : public/images/icon-192.png, icon-512.png, apple-touch-icon.png
// ═══════════════════════════════════════════════════════════════════════════
...
function encodePNG(width, height, rgba) {
  ...
  // scanlines: chaque ligne = filtre 0 + pixels
...
// ─── Mini renderer (fillRect, rounded rect, texte bitmap) ─────────────────
class Canvas {
  constructor(w, h) {
    this.w = w
    this.h = h
  ...
  // Fond dégradé diagonal sur tout le canvas
  gradient(bg) {
    for (let y = 0; y < this.h; y++)
      for (let x = 0; x < this.w; x++) {
        const t = (x + y) / (this.w + this.h)
        const r = bg[0][0] + (bg[1][0] - bg[0][0]) * t
        const g = bg[0][1] + (bg[1][1] - bg[0][1]) * t
        const b = bg[0][2] + (bg[1][2] - bg[0][2]) * t
  ...
  // Contour de rounded rect : fond de la couleur, puis intérieur de la couleur de fond
  ...
  // Police bitmap 5×7 (glyphes nécessaires au logo)
  static FONT = {
  ...
  text(text, cx, cy, cell, color) {
    ...
    const width = glyphs.reduce((acc, g) => acc + 5 * cell + cell, 0) - cell // +1 cell letter-spacing
    let x = cx - width / 2
...
// ─── Dessin du logo (coordonnées SVG 192) ─────────────────────────────────
const SVG = 192
const SHAFTS = [
...
function drawIcon(size) {
  ...
  // Fond : dégradé orange diagonal (même que le SVG)
  c.gradient([[249, 115, 22], [234, 88, 12]])
  // Coins arrondis : découpe le fond en rounded rect (fond transparent autour)
  // → on redessine sur un calque puis on masque en dehors du rounded rect
  const WHITE = [255, 255, 255, 255]
  const SHADOW = [0, 0, 0, 0]
  // Skyline des cages d'ascenseur (contours blancs)
  for (const sh of SHAFTS) {
    c.strokeRoundedRect(sh.x * s, sh.y * s, sh.w * s, sh.h * s, RADIUS * s, WHITE, SHADOW, STROKE * s)
  }
  // Ligne de base
  c.fillRect(20 * s, 160 * s - (STROKE / 2) * s, 152 * s, STROKE * s, WHITE)
  // Texte "RM" (gras, centré, y≈110)
  c.text('RM', 96 * s, (110 - 14) * s, Math.round(4 * s) || 1, WHITE)
  // Texte "FACTORY" (jaune ambre, y≈140)
  c.text('FACTORY', 96 * s, (140 - 7) * s, Math.round(2 * s) || 1, [251, 191, 36, 255])

  // Masque arrondi : tout pixel hors du rounded rect → transparent
