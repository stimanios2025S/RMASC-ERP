// ─── RMASC FACTORY — Vercel Serverless API ────────────────────────────
// FULLY SELF-CONTAINED. No TypeScript imports. Pure ESM JavaScript.
// All routes defined inline. Connects to Neon via Prisma.
...
// ─── Prisma singleton (hot cache for serverless cold starts) ───────────
...
function requireAdmin(req, res, next) {
  if (!req.user || req.user.role !== 'ADMIN') return res.status(403).json({ error: 'Accès refusé. Administrateur requis.' })
  next()
}
...
app.get('/api/health', async (_req, res) => {
  try {
    ...
    res.json({ status: 'ok', service: 'RMASC ERP', version: '2.5.3', database: 'connected' })
  } catch (e) {
    res.json({ status: 'degraded', service: 'RMASC ERP', database: 'disconnected', error: e.message })
  }
})
...
app.post('/api/users/login', async (req, res) => {
  try {
    ...
    if (!user || user.password !== password) return res.status(401).json({ error: 'Identifiant ou mot de passe incorrect.' })
...
app.post('/api/users/seed', async (_req, res) => {
  try {
    ...
    if (count > 0) return res.json({ message: 'Utilisateurs déjà initialisés.' })
    const defaults = [
    ...
    res.json({ message: `${defaults.length} utilisateurs créés.` })
  } catch (e) { res.status(500).json({ error: e.message }) }
})

app.get('/api/users', authenticate, async (_req, res) => {
  try {
    const users = await prisma.portalUser.findMany({ select: { id: true, loginId: true, name: true, role: true, canChangePassword: true } })
    res.json(users)
  } catch (e) { res.status(500).json({ error: e.message }) }
})
...
app.get('/api/orders', authenticate, async (_req, res) => {
  try {
    const orders = await prisma.order.findMany({ orderBy: { createdAt: 'desc' }, include: { _count: { select: { cadSubmissions: true } } } })
    res.json(orders)
  } catch (e) { res.status(500).json({ error: e.message }) }
})
...
app.get('/api/orders/:id', authenticate, async (req, res) => {
  try {
    const order = await prisma.order.findUnique({ where: { id: req.params.id }, include: { cadSubmissions: { orderBy: { engineeringType: 'asc' } } } })
  ...
  try {
    ...
    const order = await prisma.order.create({
      data: {
        ...
        optPanoramique: !!d.optPanoramique, optSecours: !!d.optSecours,
        optAnnoncesVocales: !!d.optAnnoncesVocales, optCctv: !!d.optCctv,
        optPortesCoupeFeu: !!d.optPortesCoupeFeu, optPanneauTactile: !!d.optPanneauTactile,
    ...
    if (e.code === 'P2002') return res.status(409).json({ error: `Le numéro de série "${req.body?.serialNumber}" existe déjà.` })
...
app.post('/api/orders/:id/approve-plan', authenticate, async (req, res) => {
  try {
    ...
    if (!order) return res.status(404).json({ error: 'Commande introuvable.' })
    if (order.status !== 'ATTENTE_APPROBATION_ADMIN') return res.status(409).json({ error: `Statut actuel: ${order.status}` })
    await prisma.order.update({ where: { id: req.params.id }, data: { status: 'ATTENTE_DESSIN_2D' } })
    res.json({ message: "Plan d'Installation approuvé." })
  } catch (e) { res.status(500).json({ error: e.message }) }
})
...
app.use((err, _req, res, _next) => {
  ...
  res.status(err.statusCode || 500).json({ error: err.message || 'Erreur interne du serveur.' })
})
