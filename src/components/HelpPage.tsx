...
export default function HelpPage({ onBack }: Props) {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-slate-800/70">
      {/* ── Header (stacks on mobile: title row + full-width search) ── */}
      <div className="sticky top-0 z-20 bg-slate-800/70 border-b border-white/10 px-3 md:px-6 py-3 flex flex-col gap-2 md:flex-row md:items-center md:justify-between shadow-sm">
        <div className="flex items-center gap-3 min-w-0">
          {onBack && (
            <button onClick={onBack} className="p-2 rounded-lg hover:bg-white/[0.06] text-white/50 hover:text-white transition-all flex-shrink-0">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-base md:text-lg font-extrabold text-white tracking-tight truncate">🧭 Catalogue d'utilisation</h1>
              <p className="text-[11px] text-white font-semibold hidden sm:block">Guide complet — RMASC FACTORY v2.5.2</p>
            </div>
          </div>
        </div>
        <div className="relative w-full md:w-64 flex-shrink-0">
          <input type="text" value={search} onChange={e => setSearch(e.target.value)}
            ...
            className="w-full px-3.5 py-2 pl-9 rounded-xl bg-slate-800/70 border border-white/10 text-sm text-white placeholder:text-white/80 focus:outline-none focus:ring-2 focus:ring-amber-200" />
