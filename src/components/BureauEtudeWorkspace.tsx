...
const ENGINEER_PAGES: EngineerPage[] = [
  { id: 'ingenieur-1', icon: '', title: 'Ingenieur Dessinateur 1', subtitle: "Plan d'Installation Technique", status: 'ATTENTE_DESSIN_TECH', nextStatus: 'ATTENTE_APPROBATION_ADMIN', nextLabel: "Envoyer le Plan a l'Administrateur", fileLabel: "Plan d'Installation", color: 'from-sky-500 to-sky-600' },
  { id: 'ingenieur-2', icon: '', title: 'Ingenieur Dessinateur 2', subtitle: 'Dessin 2D Cabine', status: 'ATTENTE_DESSIN_2D', nextStatus: 'ATTENTE_VERIFICATION', nextLabel: 'Envoyer le Dessin 2D (Verification)', fileLabel: 'Dessin 2D de la Cabine', color: 'from-violet-500 to-violet-600' },
  { id: 'verificateur', icon: '', title: 'Verificateur en Chef', subtitle: 'Controle Final et Approbation', status: 'ATTENTE_VERIFICATION', nextStatus: 'PRET_POUR_PRODUCTION', nextLabel: 'Soumettre a la Production', fileLabel: '', color: 'from-rose-500 to-rose-600' },
  { id: 'pret-prod', icon: '', title: 'Atelier de Production', subtitle: 'Commandes pretes pour la fabrication', status: 'PRET_POUR_PRODUCTION', nextStatus: null, nextLabel: null, fileLabel: '', color: 'from-emerald-500 to-emerald-600' },
]
        ...
        {engineerTabs.map(t => {
          ...
          return (
            <button key={t.id} onClick={() => { setActiveTab(t.id); setSelectedOrder(null) }}
              className={`flex items-center gap-2 px-4 py-3 text-base font-bold border-b-2 transition-all whitespace-nowrap ${isActive ? 'border-amber-400 text-white bg-amber-500/10' : 'border-transparent text-white/60 hover:text-white'}`}>
              <span>{t.icon}</span>
              <span className="hidden md:inline">{t.label}</span>
              {t.badge !== undefined && t.badge > 0 && <span className={`px-2 py-0.5 rounded-full text-[11px] font-bold ${isActive ? 'bg-amber-500 text-white' : 'bg-white/10 text-white/70'}`}>{t.badge}</span>}
            </button>
          )
        })}
      </div>
                        ...
                        <div className="bg-red-500/10 border border-red-500/25 rounded-lg px-3 py-2.5">
                          <div className="flex items-center gap-1.5 mb-1">
                            ...
                            <span className="text-[10px] font-bold uppercase tracking-wider text-red-400">
                              Plan rejete — Corrections requises
                            </span>
                          </div>
                          <p className="text-xs text-red-300 leading-relaxed">
                            {order.rejectionReason}
                          </p>
                          <div className="flex items-center gap-3 mt-1.5 text-[9px] text-red-400/70">
                            {order.rejectedBy && <span>Par : {order.rejectedBy}</span>}
                            {order.rejectedAt && <span>Le {fmtDate(order.rejectedAt)}</span>}
                          </div>
                        </div>
                      )}
                      ...
                      {order.status !== 'ATTENTE_DESSIN_TECH' && order.approvedBy && (
                        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-3 py-2.5 flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-semibold text-emerald-400">
                            Plan approuve par {order.approvedBy}
