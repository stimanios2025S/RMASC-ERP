        ...
        {uniqueStatuses.map(s => {
          ...
          return (
            <button key={s} onClick={() => setFilterStatus(s)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${filterStatus === s ? 'bg-slate-800 text-white' : 'bg-white/[0.04] border border-white/10 text-white/60 hover:bg-white/[0.04]'}`}>
              {st?.icon} {st?.label || s}
            </button>
          )
        })}
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-white/60">{order.clientName} — {order.clientCity}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="text-right">
                      <p className="text-xs font-semibold text-white">{order.typeMotorisation}</p>
                      <p className="text-[10px] text-white">Gaine: {order.largeurGaineMm}×{order.profondeurGaineMm} mm</p>
                    </div>
                  ...
                  <div className="px-5 pb-5 border-t border-slate-100 pt-4">
                          ...
                          <div key={step.key} className="flex gap-3 pb-4 last:pb-0">
                            <div className="flex flex-col items-center">
                              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${
                                isPast ? 'bg-emerald-500 text-white' : 'bg-white/[0.08] text-white'
                              }`}>
                                {isPast ? '✓' : i + 1}
                              </div>
                            ...
                            <div className="flex-1 pb-1">
                              <div className="flex items-center justify-between">
                                <p className={`text-sm font-bold ${isCurrent ? 'text-white' : isPast ? 'text-white' : 'text-white'}`}>
                                ...
                                {isPast && !isCurrent && step.hours > 0 && (
                                  <span className="text-xs text-white font-medium">✓ {step.hours}h</span>
                                )}
                              </div>
                              {isCurrent && step.hours > 0 && (
                    ...
                    <div className="mt-3 pt-3 border-t border-slate-100 grid grid-cols-2 gap-3 text-xs">
                      <div>
                        ...
                        <p className="text-white">Portes: {order.materiauPortes || '—'}</p>
                        <p className="text-white">Parois: {order.materiauParois || '—'}</p>
                        <p className="text-white">Sol: {order.materiauSol || '—'}</p>
                      </div>
                      <div>
                        <p className="font-bold text-white/60 uppercase tracking-wider text-[10px] mb-1">Motorisation</p>
                        <p className="text-white">Type: {order.typeMotorisation}</p>
                        <p className="text-white">Dimensions: {order.largeurGaineMm}×{order.profondeurGaineMm}×{order.hauteurGaineMm} mm</p>
                        <p className="text-white">Soumissions CAD: {order._count?.cadSubmissions || 0}</p>
                      </div>
                    </div>
                  </div>
                )}
