...
export default function CatalogSettings() {
  ...
  if (loading) {
    return <div className="text-center py-8 text-sm text-gray-500">Chargement du catalogue...</div>
  }
  ...
  if (categories.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-sm text-gray-400 mb-4">Le catalogue n'est pas encore initialisé.</p>
          ...
          <div className="space-y-1">
            {categories.map(cat => (
              <button key={cat.category}
                ...
                {cat.label}
                <span className="text-[10px] text-gray-500 ml-1.5">({cat.items.length})</span>
              </button>
            ))}
          </div>
          <button onClick={handleSeed}
            className="mt-4 w-full px-3 py-2 rounded-lg text-xs font-semibold bg-white/[0.06] text-gray-400 hover:bg-white/[0.1] transition-all border border-white/10">
            🔄 Réinitialiser
          </button>
        </div>
          ...
          {!selectedCat ? (
            ...
            <>
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-bold text-gray-200">{selectedCategory.label}</h3>
                <span className="text-xs text-gray-500">{selectedCategory.items.length} élément{selectedCategory.items.length > 1 ? 's' : ''}</span>
              </div>
                  ...
                  .map(item => (
                    <div key={item.value}
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-200 truncate">{item.label}</p>
                        {item.desc && <p className="text-[10px] text-gray-500 truncate">{item.desc}</p>}
                        <p className="text-[9px] text-gray-400 font-mono">{item.value}</p>
                      </div>
              ...
              <div className="bg-white/[0.04] border border-white/10 rounded-xl p-4">
                <p className="text-xs font-bold text-gray-400 mb-3">➕ Ajouter un élément</p>
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div>
                    <label className="text-[10px] font-medium text-gray-500 block mb-1">Valeur technique *</label>
                  ...
                  <div>
                    <label className="text-[10px] font-medium text-gray-500 block mb-1">Libellé affiché *</label>
                    <input type="text" value={editLabel} onChange={e => setEditLabel(e.target.value)}
                      placeholder="Ex: Nouveau Matériau"
                      className="w-full px-3 py-2 rounded-lg bg-white/[0.06] border border-white/10 text-sm text-gray-200 placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-amber-500/30" />
                  </div>
                  <div className="col-span-2">
                    <label className="text-[10px] font-medium text-gray-500 block mb-1">Description (optionnelle)</label>
