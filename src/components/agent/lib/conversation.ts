...
const FLOWS: Record<string, { title: string; steps: { key: string; question: string }[]; totalSteps: number }> = {
  create_order: {
    ...
    steps: [
      ...
      { key: 'typeMotorisation', question: "⚡ **Type d'ascenseur** — Électrique ou hydraulique ?" },
      { key: 'sousTypeElectrique', question: "🔌 **Type de moteur** — Si c'est électrique, précise (ex: 2 vitesses, variateur) ? (sinon tape 'aucun')" },
      { key: 'dimensions', question: "📏 **Dimensions** — Largeur, profondeur et hauteur de la gaine en mm ? (ex: 2000x1800x25000 ou 'je sais pas')" },
      { key: 'nombreEtages', question: "🏗️ **Nombre d'arrêts** — Combien d'arrêts desservis ?" },
...
export async function processFlowStep(
  ...
  if (nextStepIndex >= flow.totalSteps) {
    ...
    const summaryLines: string[] = [
      ...
      ``,
      `⚡ **Type d'ascenseur :** ${newData.typeMotorisation || '—'}`,
      `🔌 **Type de moteur :** ${newData.sousTypeElectrique || 'Standard'}`,
      `📏 **Gaine :** ${newData.largeurGaineMm || '?'} x ${newData.profondeurGaineMm || '?'} x ${newData.hauteurGaineMm || '?'} mm`,
      `🏗️ **Nombre d'arrêts :** ${newData.nombreEtages || '—'}`,
