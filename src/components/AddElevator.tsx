...
function FormInput({ label, value, onChange, type = 'text', placeholder, required = false, optional = false }: any) {
  return <div className="flex flex-col gap-1.5">
    <label className="text-sm font-medium text-white flex items-center gap-1.5">{label}{required && <span className="text-red-400 text-xs">*</span>}{optional && <span className="text-white text-[11px] font-normal italic">(Optionnel)</span>}</label>
    <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
      className="w-full h-10 px-3.5 rounded-xl border bg-slate-700/60 text-sm text-gray-100 placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500/50 transition-all border-slate-600/50"/>
  </div>
}
function FormSelect({ label, value, onChange, options, required = false }: any) {
  return <div className="flex flex-col gap-1.5">
    <label className="text-sm font-medium text-white flex items-center gap-1.5">{label}{required && <span className="text-red-400 text-xs">*</span>}</label>
...
function FormSelectRich({ label, value, onChange, options, required = false }: any) {
  ...
  return <div className="flex flex-col gap-1.5">
    ...
    {s && <p className="text-[11px] text-white italic mt-0.5 leading-tight">{s.desc}</p>}
  </div>
}
  ...
  return <div className="flex flex-col gap-1.5">
    ...
    {isAutre && (
      <input type="text" value={customValue} onChange={e => onCustomChange(e.target.value)}
        ...
        className="w-full h-10 px-3.5 rounded-xl border border-amber-500/40 bg-slate-800/80 text-sm text-gray-100 placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-amber-500/40 focus:border-amber-500/60 transition-all" />
    )}
  </div>
}
...
function FormSelectRichWithAutre({ label, value, onChange, options, customValue, onCustomChange, placeholder, required = false }: any) {
  ...
  return <div className="flex flex-col gap-1.5">
    ...
    {!isAutre && s && <p className="text-[11px] text-white italic mt-0.5 leading-tight">{s.desc}</p>}
    {isAutre && (
      <input type="text" value={customValue} onChange={e => onCustomChange(e.target.value)}
        placeholder={placeholder || 'Saisir la valeur personnalisee...'}
        className="w-full h-10 px-3.5 rounded-xl border border-amber-500/40 bg-slate-800/80 text-sm text-gray-100 placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-amber-500/40 focus:border-amber-500/60 transition-all" />
          ...
          <div className="flex gap-2 h-10">
            {PRIORITY_OPTIONS.map(p => (
              <button key={p.value} type="button" onClick={() => setData({ ...data, priority: p.value })}
                className={`flex-1 rounded-xl text-xs font-bold border-2 transition-all ${data.priority === p.value ? `${p.color} border-current` : 'border-slate-600/50 text-white bg-slate-700/40 hover:border-slate-500'}`}>
                {p.label}
              </button>
            ))}
          </div>
    ...
    {isActive && <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border border-slate-700 rounded-2xl p-5 space-y-4 shadow-2xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div><span className="text-xs font-bold uppercase tracking-wider text-white">Salim Hamoun AI v1.0</span>
        ...
        <div className="hidden sm:flex flex-col items-end gap-1">
          <span className="text-[9px] bg-slate-800 text-white/80 px-2 py-0.5 rounded-full font-mono">CP {positionCp === 'Fond' ? 'au Fond' : 'Latéral'}</span>
          <span className="text-[9px] bg-slate-800 text-white/80 px-2 py-0.5 rounded-full font-mono">Porte {porteType} · {moteurType}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="bg-slate-800/60 rounded-xl px-4 py-3.5 border border-white/10">
          ...
          <p className="text-[10px] text-white/50 mt-1 font-mono leading-relaxed">{deductionDetails[0]}</p>
        ...
        <div className="bg-slate-800/60 rounded-xl px-4 py-3.5 border border-white/10">
          <div className="flex items-center justify-between mb-2"><span className="text-xs font-semibold text-white">Profondeur cabine (Pc)</span><span className="text-[10px] text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-full">⚙️ Calcul normé</span></div>
          <p className="text-2xl font-bold text-white tracking-tight">{estimatedCabinDepth}</p>
          <p className="text-[10px] text-white/50 mt-1 font-mono leading-relaxed">{deductionDetails[1]}</p>
          {penaltyApplied && <p className="text-[9px] text-amber-400/70 mt-0.5 font-mono">{[penaltyGB > 0 && `Gearbox −${penaltyGB}`, penaltyH > 0 && `Hydraulique −${penaltyH}`].filter(Boolean).join(' + ')}</p>}
        </div>
      </div>
      <div className="flex flex-wrap gap-1.5 pt-1">
        <span className="text-[8px] bg-slate-800 text-white/50 px-2 py-0.5 rounded-full font-mono border border-slate-700/40">E_cp={E_CP}</span>
        <span className="text-[8px] bg-slate-800 text-white/50 px-2 py-0.5 rounded-full font-mono border border-slate-700/40">E_guides={E_GUIDES_PATTES}</span>
        <span className="text-[8px] bg-slate-800 text-white/50 px-2 py-0.5 rounded-full font-mono border border-slate-700/40">J_sécu={J_SECURITE}</span>
        <span className="text-[8px] bg-slate-800 text-white/50 px-2 py-0.5 rounded-full font-mono border border-slate-700/40">E_porte_{porteType === 'Centrale' ? 'cent' : porteType === 'Battante' ? 'batt' : 'télesc'}={porteType === 'Centrale' ? E_PORTE_CENTRALE : porteType === 'Battante' ? E_PORTE_BATTANTE : E_PORTE_TELESCOPIQUE}</span>
        <span className="text-[8px] bg-slate-800 text-white/50 px-2 py-0.5 rounded-full font-mono border border-slate-700/40">Rés.tech={E_RESERVE_TECHNIQUE}</span>
        {penaltyApplied && <span className="text-[8px] bg-slate-800 text-white/50 px-2 py-0.5 rounded-full font-mono border border-slate-700/40">Pén: {[penaltyGB > 0 && `GB-${penaltyGB}`, penaltyH > 0 && `HY-${penaltyH}`].filter(Boolean).join(' ')}</span>}
      </div>
    </div>}
...
function StepFinalisation({ data, setData }: any) {
  ...
  return <div className="space-y-6">
    <div className="flex items-center gap-2 mb-1"><Icon name="FileCheck" className="w-5 h-5 text-amber-400" /><p className="text-sm font-semibold text-white">Récapitulatif</p></div>
    <div className="space-y-4">{sections.map((section, si) => <div key={si}><h4 className="text-xs font-bold uppercase tracking-wider text-white mb-2">{section.title}</h4><div className="border border-slate-600/50 rounded-xl divide-y divide-slate-600/30 bg-slate-700/40">{section.rows.length === 0 ? <p className="px-4 py-2.5 text-sm text-white italic">—</p> : section.rows.map((row: any, ri: number) => <div key={ri} className="flex items-center justify-between px-4 py-2.5"><span className="text-sm text-white">{row.label}</span><span className="text-sm font-semibold text-white">{row.value}</span></div>)}</div></div>)}</div>
    {/* ─── Notes ─── */}
    <div>
      <label className="text-xs font-bold uppercase tracking-wider text-white mb-2 block">📝 Notes complémentaires (optionnel)</label>
      <textarea value={data.notes} onChange={e => setData({ ...data, notes: e.target.value })}
        ...
        rows={3}
        className="w-full px-3.5 py-2.5 rounded-xl border border-slate-600/50 text-sm text-gray-100 bg-slate-700/60 placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-amber-500/30 resize-none" />
    </div>
    <hr className="border-slate-600/30" />
    <div className="space-y-3">
      <div onClick={() => setData({ ...data, agreed: !data.agreed })} className="flex items-start gap-3 p-4 rounded-xl border-2 border-slate-600/50 bg-slate-700/40 hover:bg-slate-700/60 transition-all cursor-pointer select-none">
        ...
        <span className="text-sm font-medium text-white leading-relaxed">Je confirme que toutes les informations sont correctes.</span>
      </div>
      <p className="text-[11px] text-white/50 text-center">Le numéro de série sera généré automatiquement.</p>
    </div>
  </div>
}
...
export default function AddElevator({ onBack, editOrder }: Props) {
  ...
  if (submitting) return <div className="flex-1 flex flex-col items-center justify-center p-12">
    ...
    <h2 className="text-xl font-bold text-white mb-3">Traitement en cours</h2>
    <p className="text-xs text-white/50">Enregistrement de la commande...</p>
  </div>
  ...
  if (submitError) return <div className="flex-1 flex flex-col items-center justify-center p-12">
    ...
    <div className="flex gap-3">
      ...
      <button onClick={onBack} className="px-5 py-2.5 rounded-xl border border-white/10 text-white text-sm font-semibold">← Retour</button>
    </div>
  </div>
  ...
  if (saved) return <div className="flex-1 flex flex-col items-center justify-center p-12">
    ...
    <p className="text-sm text-white/50 mb-8">La commande a été transmise au bureau d'étude.</p>
